...
namespace GraphLibraryTest
{
	...
	{ CREATE_MEMORY_SNAPSHOT{
		GraphBuilder gb{};
		gb.add_node(L"const-func", FunctionConstant::NAME);
		Graph g = gb.build();
	...
	{ CREATE_MEMORY_SNAPSHOT{
		GraphBuilder gb{};
		auto n0 = gb.add_node(L"const", FunctionConstant::NAME, 1);
		auto n1 = gb.add_node(L"oppos", FunctionOpposite::NAME);
		gb.add_edge(n0, n1);
		Graph g = gb.build();
	...
	{ CREATE_MEMORY_SNAPSHOT {
		GraphBuilder gb{};
		auto node_1 = gb.add_node(L"1", FunctionConstant::NAME, 1);
	...
	{ CREATE_MEMORY_SNAPSHOT {
		...
		auto node_1 = gb.add_node(L"1", FunctionConstant::NAME, 1);
		auto node_2 = gb.add_node(L"2", FunctionConstant::NAME, 2);
		auto node_3 = gb.add_node(L"3", FunctionConstant::NAME, 3);
		auto node_4 = gb.add_node(L"4", FunctionConstant::NAME, 4);
	...
	{ CREATE_MEMORY_SNAPSHOT {
		GraphBuilder gb{};
		auto node_1 = gb.add_node(L"9", FunctionConstant::NAME, 1);
	...
	TEST(graphBuilderTests, TestGraphOperatorReciprocal)
	{ CREATE_MEMORY_SNAPSHOT {
		GraphBuilder gb{};
		auto node_9 = gb.add_node(L"9", FunctionConstant::NAME, 9);
		auto node_3 = gb.add_node(L"3", FunctionConstant::NAME, 3);
		auto node_result = node_9 / node_3;
		Graph g = gb.build();

		EXPECT_EQ(4, g.order()) << "Graph order";
		EXPECT_EQ(3, g.size()) << "Graph size";

		{
			auto input_nodes = g.find_input_nodes();
			EXPECT_EQ(2, input_nodes.size()) << "input node's size";
			EXPECT_EQ(L"9", input_nodes[0]->label()) << "node 0 label";
			EXPECT_EQ(L"3", input_nodes[1]->label()) << "node 1 label";
			EXPECT_EQ(FunctionConstant::NAME, input_nodes[0]->function().name()) << "node label";
			EXPECT_EQ(FunctionConstant::NAME, input_nodes[1]->function().name()) << "node label";
		}
		{
			auto output_nodes = g.find_output_nodes();
			EXPECT_EQ(1, output_nodes.size()) << "output node's size";
			EXPECT_EQ(L"(9)/(3)", output_nodes[0]->label()) << "node sqrt label";
			EXPECT_EQ(FunctionMultiply::NAME, output_nodes[0]->function().name()) << "node multiply function name";
		}
		// move graph
		Graph moved_graph = std::move(g);
		EXPECT_EQ(4, moved_graph.order()) << "Graph order";
		EXPECT_EQ(3, moved_graph.size()) << "Graph size";

		Graph moved_constructed(std::move(moved_graph));
		EXPECT_EQ(4, moved_constructed.order()) << "Graph order";
		EXPECT_EQ(3, moved_constructed.size()) << "Graph size";
	} ASSERT_MEMORY_SNAPSHOT }
}